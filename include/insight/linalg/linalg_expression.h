...
#ifndef INCLUDE_INSIGHT_LINALG_LINALG_EXPRESSION_H_
#define INCLUDE_INSIGHT_LINALG_LINALG_EXPRESSION_H_
...
// Base class for all linear algebra expressions.
template<typename Derived>
struct linalg_expression {
  const Derived& self() const { return static_cast<const Derived&>(*this); }
};

// Binary expression.

// Element-wise arithmetic between two generic linear algebra expressions.
template<typename E1, typename E2, typename Function>
struct binary_expression :
      public linalg_expression< binary_expression<E1, E2, Function> > {
  ...
  binary_expression(const E1& e1, const E2& e2, const Function& f)
      : e1(e1), e2(e2), f(f) {
...
// Element-wise arithmetic between a generic linear algebra expression
// and a scalar.
template<typename E, typename Function>
struct binary_expression<E, typename E::value_type, Function>
    : public linalg_expression<binary_expression<E, typename E::value_type,
                                                 Function> > {
  ...
  binary_expression(const E& e, value_type scalar, const Function& f)
...
// Element-wise arithmetic between a scalar and a generic linear algebra
// expression.
template<typename E, typename Function>
struct binary_expression<typename E::value_type, E, Function>
    : public linalg_expression<binary_expression<typename E::value_type, E,
                                                 Function> > {
  using size_type = typename E::size_type;
  using value_type = typename E::value_type;
  using shape_type = typename E::shape_type;
  ...
  binary_expression(value_type scalar, const E& e, const Function& f)
...
// Addition between two generic linear algebra expressions.
...
std::enable_if<
  ...
  >::type
operator+(const linalg_expression<L>& e1, const linalg_expression<R>& e2) {
...
// addition between a generic linear algebra expression and a scalar.
template<typename E>
inline
binary_expression<E, typename E::value_type,
                  std::plus<typename E::value_type> >
operator+(const linalg_expression<E>& e, typename E::value_type scalar) {
  return binary_expression<
    E,
...
// addition between a scalar and a generic linear algebra expression.
template<typename E>
inline
binary_expression<typename E::value_type, E,
                  std::plus<typename E::value_type> >
operator+(typename E::value_type scalar, const linalg_expression<E>& e) {
  return binary_expression<
    typename E::value_type,
    E,
    std::plus<typename E::value_type>
...
// Substraction between two generic linear algebra expressions.
template<typename L, typename R>
inline
// We need to make sure that whose ever L and R are they must have the same
...
// Substraction between a generic linear algebra expression and a scalar.
template<typename E>
inline
binary_expression<E, typename E::value_type,
...
// Substraction between a scalar and a generic linear algebra expression.
template<typename E>
inline
binary_expression<typename E::value_type, E,
                  std::minus<typename E::value_type> >
...
// Element-wise multiplication between two generic linear algebra
// expressions.
template<typename L, typename R>
inline
// We need to make sure that whose ever L and R are they must have the same
...
// Multiplication between a generic linear algebra expression and a scalar.
template<typename E>
inline
binary_expression<E, typename E::value_type,
...
// Multiplication between a scalar and a generic linear algebra expression.
template<typename E>
inline
binary_expression<typename E::value_type, E,
            std::multiplies<typename E::value_type> >
...
// Element-wise division between two generic linear algebra expressions.
template<typename L, typename R>
inline
// We need to make sure that whose ever L and R are they must have the same
// element type. So that, for example no viable overload for element-wise
...
// Element-wise division between a generic linear algebra expression and
// a scalar.
template<typename E>
inline
binary_expression<E, typename E::value_type,
...
// Element-wise division between a scalar and a generic linear algebra
// expression.
template<typename E>
inline
binary_expression<typename E::value_type, E,
...
#endif  // INCLUDE_INSIGHT_LINALG_LINALG_EXPRESSION_H_
